...
class EqualFall(Scene):
    def construct(self):
        ...
        vacuum_text2 = Text("Both the objects reach the ground at the same time!").scale(.75)
        ...
        ground_y = ground.get_top()[1]

        feather_target_y = ground_y + feather.height / 2
        ball_target_y = ground_y + ball.height / 2

        self.play(
            feather.animate.set_y(feather_target_y),
            ball.animate.set_y(ball_target_y),
            run_time=2,
            rate_func=linear
        )
        self.play(Write(vacuum_text2), run_time=0.5)
        self.wait(2)
        self.play(FadeOut(vacuum_text))
        self.play(FadeOut(vacuum_text2))
        ...
        outro = Text("By \nPranshu Rawat - 25BTCS100\nNikhil Mishra - 25BTCS088")
